...
def load_to_postgres():
    ...
    engine = create_engine(
        "postgresql://user_tcc:password_tcc@localhost:5432/movielens_db"
    )
    ...
    movies_df[["movieId", "title"]].to_sql(
        "filmes", engine, if_exists="replace", index=False
    )

    print(" -> Inserindo Relações Filme-Gênero...")
    filme_genero_completo = filme_genero_df.merge(
        generos_df, left_on="genre", right_on="nome_genero"
    )
